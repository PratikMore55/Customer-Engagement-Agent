...
// This replaces ai.service.js without needing API keys
...
class MockAIService {
   ...
   * @param {number} maxTokens - Max tokens (ignored in mock)
   ...
   * @param {string} systemPrompt - System instructions
   * @param {string} userPrompt - User message
   * @returns {Promise<object>} - Simulated structured response
   */
  ...
  generateMockClassification(userPrompt) {
    ...
    const hasBudgetMention = userPrompt.toLowerCase().includes('budget');
    const hasUrgentMention = userPrompt.toLowerCase().includes('urgent') || 
                            userPrompt.toLowerCase().includes('immediate') ||
                            userPrompt.toLowerCase().includes('asap');
    const hasTimelineMention = userPrompt.toLowerCase().includes('timeline') ||
                              userPrompt.toLowerCase().includes('when');
